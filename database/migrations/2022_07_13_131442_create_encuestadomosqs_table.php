...
class CreateEncuestadomosqsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('encuestadomosqs', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('idMonitoreo')->unsigned();
            $table->foreign('idMonitoreo')->references('id')->on('monitoevalu_mosqs')->onUpdate('cascade')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('encuestadomosqs');
    }
}
